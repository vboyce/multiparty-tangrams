...
# TODOs
 ...
 figure out models to re-run

check what current empirica citaiton is
 Models to do:
 Accuracy as funct of round and size
 DItto speed
 When listeners talk
 When speakers talk (word/block, words/player)
 SPeaker wrong last time analysis
 Content similarity (for last round? for any pair??, include same speaker part)
...
## Speed and Accuracy
Most groups were accurate in their selections, with accuracy rising over blocks (Fig \ref{accuracy}). This indicates that speakers were usually successful at conveying the intended referents. Participants are more accurate in later blocks.  TODO comment on trend (and stats?)
...
Participants selected images faster in later blocks (Fig \ref{time}), although there is wide variability. This speed up is consistent with prior work by @weberCulturalConflictMerger2003 which used speed as the dependent measure.
