---
title: "Two's company, six is chaos: emergence of conventions in reference games"
...
```{r accuracy, fig.env="figure", fig.pos = "H", fig.align = "center", out.width="100%", fig.width=6, fig.height=3, fig.cap = "Accuracy TODO \\label{accuracy}" }
...
geom_smooth(method = "glm", method.args = list(family = "binomial"), size=1.3) + 
  stat_summary(fun.data = "mean_cl_boot", position = position_dodge(width=.3), size=.3)+
  scale_color_viridis(discrete=T, direction=-1)+
    scale_x_continuous(breaks=seq(1,6))+
...
```{r time, fig.env="figure", fig.pos = "H", fig.align = "center",out.width="100%", fig.width=6, fig.height=3, fig.cap = "Listeners selected images faster in later blocks. Only times to correct responses are shown.\\label{time}" }
d.round_results %>% group_by(playerId, repNum, gameId, numPlayers) %>% 
  filter(correct==T) %>% 
...
geom_smooth(method = "glm", formula = y~x,
 ...
 scale_color_viridis(discrete=T, direction=-1)+ 
  labs(x="Round", y="Time to select (s)", color="Player count")+
  theme(legend.position = "right")
...
```{r words, fig.env = "figure*", fig.pos = "h", out.width="100%", fig.width=8, fig.height=2, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "Speaker and listeners say fewer words in later blocks. Note: y-axis clipped at 50 which hides a few speaker outliers. \\label{words}"}
