...
summary <- combined_results %>% group_by(condition, trialNum, repNum, gameId, numPlayers) %>% 
  ...
  arrange(condition, Players,Complete,Partial)
...
combined_results %>% filter(condition=="rotate") |> group_by(playerId,repNum, gameId, numPlayers) %>% 
...
combined_chat %>%filter(condition=="rotate") |>  filter(role=="speaker") %>% group_by(gameId,numPlayers,repNum) %>% 
...
```{r}
library(cowplot)
palette = c("2p rotate"="gray70","6p rotate"="gray40", "6p no rotate"="orange", "6p full feedback"="pink", "6p emoji"="red")
for_camp <- combined_chat  |> filter(role=="speaker") %>% 
...
ro <- for_camp |> filter(condition %in% c("2p rotate", "6p rotate", "6p no rotate")) |> ggplot(aes(x=repNum+1, y=words, color=rotate))+
   scale_color_manual(values=palette, breaks=c("2p rotate", "6p rotate", "6p no rotate"))+
...
feedback <- for_camp |> filter(rotate %in% c("2p rotate", "6p rotate", "6p full feedback")) |> ggplot(aes(x=repNum+1, y=words, color=rotate))+
...
emoji <- for_camp |> filter(rotate %in% c("2p rotate", "6p rotate", "6p emoji")) |> ggplot(aes(x=repNum+1, y=words, color=rotate))+
   scale_color_manual(values=palette)+
      geom_point(size=1, alpha=.05, position = position_dodge(width=.4))+
    geom_smooth(method=glm, formula=y~poly(x,2), alpha=.3)+
  coord_cartesian(ylim=c(0,60))+
...
orig <- rod.chat |>  filter(role=="speaker") %>% mutate(groupxtangram=str_c(gameId,tangram)) %>% 
  group_by(repNum, gameId,tangram, groupxtangram, numPlayers) %>% 
  summarize(words=sum(total_num_words))

rotate <- orig  |> ggplot(aes(x=repNum+1, y=words, color=as.factor(numPlayers)))+
    scale_color_viridis(discrete=T, direction=-1)+
      geom_point(size=1, alpha=.05, position = position_dodge(width=.4))+
    geom_smooth(method=glm, formula=y~poly(x,2), alpha=.3)+
  coord_cartesian(ylim=c(0,60))+
...
combined_results |> filter(condition %in% c("no_rotate","full_feedback", "emoji", "rotate")) %>%
  filter(numPlayers==6) |> 
...
#3
 combined_results |> filter(condition %in% c("2_thin", "2_thick", "6_thin", "6_thick")) %>% group_by(playerId,repNum, gameId, condition) %>% 
  filter(response!="false") |> 
  mutate(correct.num=ifelse(correct,1,0)) %>% 
  ggplot(aes(x=repNum+1, y=correct.num, color=condition))+
     scale_x_continuous(breaks=seq(1,6))+
...
combined_chat |> filter(condition=="rotate") |>  filter(role=="speaker") %>% 
    mutate(groupxtangram=str_c(gameId,tangram)) %>% 
  group_by(repNum, gameId,tangram, groupxtangram, numPlayers) %>% 
  summarize(words=sum(total_num_words)) |> 
...
palette = c("no_rotate"="orange", "full_feedback"="pink", "emoji"="red","rotate"="purple")
combined_chat|> filter(condition %in% c("no_rotate","full_feedback", "emoji", "rotate")) %>%
  filter(numPlayers==6) |> 
    mutate(groupxtangram=str_c(gameId,tangram)) %>% 
  group_by(repNum, gameId,tangram, groupxtangram,condition) %>% 
  summarize(words=sum(total_num_words)) |> 
...
combined_chat |> filter(condition %in% c("2_thin", "2_thick", "6_thin", "6_thick")) %>% filter(role=="speaker") %>% 
    mutate(groupxtangram=str_c(gameId,tangram)) %>% 
  group_by(repNum, gameId,tangram, groupxtangram, condition) %>% 
  summarize(words=sum(total_num_words)) |> 
  #group_by(repNum, name, gameId) |> 
...
```{r}
# TODO figure out where the converge & diverge for tgrams is!!!
# and relocate those troubled ones from code to somewhere reasonable like data!
...
one_two_diverge <- do_diverge(one_two_sbert_concat)
one_two_converge <- do_converge(one_two_sbert_concat)
three_diverge <- do_diverge(three_sbert_concat)
...
#1
 one_two_diverge |> filter(condition %in% c("2", "3","4","5","6")) |> 
...
palette = c("6noro"="orange", "6highfeed"="pink", "6emoji"="red","6"="purple")
 ...
 one_two_diverge |>  filter(condition %in% c("6emoji", "6highfeed", "6noro","6")) |> 
                              ggplot( aes(x=repNum+1,y=sim,color=condition))+
      stat_summary(aes(group=str_c(condition,tangram), color=condition),fun.data = "mean_cl_boot", position = position_dodge(width=.3), alpha=.5, geom="point")+
  geom_smooth(formula=y~poly(x,2))+
  scale_x_continuous(breaks=seq(1,6))+
...
#3
 three_diverge |> ggplot(aes(x=repNum+1,y=sim,color=condition))+
      stat_summary(aes(group=str_c(condition,tangram), color=condition),fun.data = "mean_cl_boot", position = position_dodge(width=.3), alpha=.5, geom="point")+
  geom_smooth(formula=y~poly(x,2))+
  scale_x_continuous(breaks=seq(1,6))+
   coord_cartesian(ylim=c(0,1))+
...
#1
one_two_converge |> filter(condition %in% c("2", "3","4","5","6")) |> ggplot( aes(x=earlier+1,y=sim,color=condition))+
...
one_two_converge |>  filter(condition %in% c("6emoji", "6highfeed", "6noro","6")) |> ggplot( aes(x=earlier+1,y=sim,color=condition))+
      stat_summary(aes(group=str_c(gameId), color=condition),fun.data = "mean_cl_boot", position = position_dodge(width=.3), alpha=.5, geom="point")+
  #geom_smooth(formula=y~poly(x,2))+ # the smooth isn't working b/c singularities
  scale_x_continuous(breaks=seq(1,6))+
  labs(y="Cosine Similarity", x="Block", color="")+
  theme(legend.position="bottom",
...
three_converge |>  ggplot( aes(x=earlier+1,y=sim,color=condition))+
      stat_summary(aes(group=str_c(gameId), color=condition),fun.data = "mean_cl_boot", position = position_dodge(width=.3), alpha=.5, geom="point")+
  geom_smooth(formula=y~poly(x,2))+
  scale_x_continuous(breaks=seq(1,6))+
  labs(y="Cosine Similarity", x="Block", color="")+
