...
Listeners often don't talk much, but are more likely to ask questions or make clarification in early blocks. In a linear regression for the number of words each listener said ( `r form(listener_spec)`), there was an effect of block (`r stats(listener, 1)`), but no clear effect of game size (`r stats(listener,4)`). 
...
```{r} 
read_rds(here(model_location,"acc_model_study3_mix.rds")) |> show_summary()
...
```{r} 
red_3 <- read_rds(here(model_location,"red_model_study3_mix.rds")) |> show_summary()

```

This model was run with 2 player thick as the default. The effect of being one block later for 2 player thick games is `r stats_text(red_3, 5)`. TODO

## Sbert models

#### Divergence
```{r} 
read_rds(here(model_location,"div_model_study3_mix.rds")) |> show_summary()
...
```{r}
# read_rds(here(model_location,"tangram_model_study3_mix.rds")) |> summary() # do tangrams get more distinctive I think 

read_rds(here(model_location,"to_last_model_study3.rds")) |> show_summary() #TODO has over-parameter issues with same speaker!!!
