...
adhoc conventions needed when priors are more different @murthy2022
---
...
geom_smooth(method = "glm", method.args = list(family = "binomial")) + 
  ...
  #guides(color = guide_legend(nrow=1, byrow=F, override.aes = list(linetype = 0, alpha=1, fill=NA, size=1.2) ) )+
...
<!--[TODO this paragraph was a bit tough to follow, as it goes through the three experiments very quickly and it's easy to forget what the experiment numbers correspond to. I would re-frame the write up the results in terms of the *factors we're manipulating* (i.e. the axes in Fig. 1A) and then put the experiment ID in parentheses, like "we found a large effect of *group size*, where larger groups were initially less accurate ... (Exp. 3; \beta = ..., ...)"]-->
...
Group size did not have a strong effect on initial accuracy or improvement rate (`r stats(acc_1,2)`) in the baseline condition (expt 1, `r stats(acc_1,4)`). 6 player thick games had lower initial accuracy  (expt 3, `r stats(acc_3, 6)`)  and were slower to improve were slower to improve than 2-person thick games (expt 3, `r stats(acc_3, 3)`).

Initial accuracy was somewhat higher for consistent speaker games (`r stats(acc_2a,2)`) and full feedback games (`r stats(acc_2b, 2)`) than for 6 thin games (expt 2, `r stats(acc_2c, 2)`), and improvement rates were higher for consistent speaker games (`r stats(acc_2a,1)`) and full feedback games (`r stats(acc_2b, 1)`) than for 6 thin games (expt 2, `r stats(acc_2c, 1)`). Thick and thin 2-player games were not reliably different in their initial accuracy (expt 3, `r stats(acc_3,5)`) or improvement rate (expt 3, `r stats(acc_3,2)`). There was no interaction between thinness and group size on initial condition (`r stats(acc_3, 7)`) or improvement rate (`r stats(acc_3, 4)`).  Across experiments, the high and increasing levels of accuracy indicate that across all of these conditions, participants are able to succeed in communicating about the images. 
...
<!--### Speaker utterances shorten over repetition

Speakers in larger games were more verbose than speakers in smaller games, and in some cases, these speakers showed sharper reduction from initial wordiness to eventual concision (Figure \@ref(fig:behavioral)B). We constructed linear models of the number of words a speaker said each trial as a function of condition and block. Speakers in larger groups said more to start with than speakers in smaller groups (expt 1, `r stats_text(red_1,4)`; expt 3, `r stats(red_3,7)`). Thin games were similar to thick games in initial verbosity (expt 3, `r stats(red_3, 5)`). 
  
The numbers of words said decreased at a similar rate across group size in Experiment 1 (`r stats(red_1,2)`), but 6-player games decreased faster than 2-player games in Experiment 3 (`r stats(red_3, 4)`). Thin games were similar to thick games in reduction rate (expt 3, `r stats(red_3, 2)`). 

The main effect of being one block later was `r stats_text(red_1,1)` words per trial in Experiment 1; `r stats_text(red_2a,1)` words per trial for 6 consistent speaker; `r stats_text(red_2b,1)` words for 6 full feedback, `r stats_text(red_2c,1)` words for 6 thin; and `r stats(red_3,1)` for the 2 thick condition of experiment 3. Overall, all conditions exhibited reduction, with variations in the speed at which speaker utterances got shorter. 
-->
### Speaker utterances shorten over repetition

Speakers in larger games were more verbose than speakers in smaller games, and in some cases, these speakers showed sharper reduction from initial wordiness to eventual concision (Figure \@ref(fig:behavioral)B). We constructed linear models of the number of words a speaker said each trial as a function of condition and block. 

In the initial round, speakers in larger groups said more than speakers in smaller groups (expt 1, `r stats(red_1,4)`; expt 3, `r stats(red_3,7)`). The rate of reduction was relatively similar across group size for baseline games (expt 1, `r stats(red_1,2)`), but 6-player games reduced faster than 2-player games in thick games (`r stats(red_3, 4)`). Taken together, smaller games on average said less at each stage of the game than larger games, although the gap closed in later rounds. Thin 2-player games were similar to thick 2-player games in initial verbosity (expt 3, `r stats(red_3, 5)`) and in reduction rate (expt 3, `r stats(red_3, 2)`). There was no effect of the interaction between thinness and game size on initial verbosity (`r stats(red_3, 6)`) or reduction rate (`r stats(red_3,3)`). 

The main effect of being one block later was `r stats_text(red_1,1)` words per trial in Experiment 1; `r stats_text(red_2a,1)` words per trial for 6 consistent speaker; `r stats_text(red_2b,1)` words for 6 full feedback, `r stats_text(red_2c,1)` words for 6 thin; and `r stats(red_3,1)` for 2-player thick. Overall, all conditions exhibited reduction, with some variation in the slope of reduction. 
...
Over repetitions, speaker descriptions diverged from descriptions used in other groups: divergence was fastest in groups with thick communication channels, while the 6-player thin condition games barely diverged at all (Figure \@ref(fig:sbert)B). We modeled semantic divergence by looking at the similarity between a pair of utterances for the same image from the same block across different games as a function of the block number and condition. Cross-group similarities started off the same regardless of group size (expt 1, `r stats(div_1, 3,3)`), but larger groups diverged from each other slightly slower than smaller groups (expt 1,`r stats(div_1,2,3)`). Divergence was stronger in the consistent speaker (expt 2,`r stats(div_2a, 1,3)`) and full feedback conditions (expt 2, `r stats(div_2b, 1,3)`) than in the 6 thin condition (expt 2, `r stats(div_2c, 1,3)`). The 2-player thick condition diverged at a moderate speed (expt 3, `r stats(div_3, 1,3)`), and the 6-player thick condition had initially higher similarity (expt 3, `r stats(div_3, 7,3)` and faster divergence (expt 3, `r stats(div_3, 4,3)` in comparison. Compared to the two-player thick games, two-player thin games started with slightly higher similarity (expt 3, `r stats(div_3, 5, 3)`) and diverged slightly more slowly (expt 3, `r stats(div_3, 2,3)`). Most noticeably, the 6 player thin games diverged much more slowly than the other conditions (expt 3, `r stats(div_3, 3,3)`). -->
...
Game size had inconsistent effects on divergence slope: in baseline games, smaller games diverged faster than larger games (expt 1,`r stats(div_1,2,3)`) from the same starting point (expt 1, `r stats(div_1, 3,3)`).  In thick games, 6-player games diverged faster than 2-player games (expt 3, `r stats(div_3, 4,3)`) but from initially higher cross-game similarity (expt 3, `r stats(div_3, 7,3)`). In both cases, smaller games were less similar to each other on average.  

Thicker games tended to diverge faster than thinner games. Both consistent speaker (expt 2, `r stats(div_2a, 1,3)`) and full feedback games (expt 2, `r stats(div_2b, 1,3)`) diverged faster than thin games. Thick 2-person games diverged faster than thin 2-person games (expt 3, `r stats(div_3, 2,3)`). Most noticeably, the 6 player thin games diverged much more slowly than the other conditions (expt 3, `r stats(div_3, 3,3)`). 
...
### Different patterns of performance and content
