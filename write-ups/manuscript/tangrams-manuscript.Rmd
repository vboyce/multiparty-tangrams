...
In Experiment 3, larger games began with lower initial accuracy (`r stats(acc_3,6)`) and improved more slowly (`r stats(acc_3,6)`) than smaller games, although group size differences were not reliable in Experiment 1 (SI Table 4). 
...
listener_sample <- pre_chat_3 |> filter(target=="/experiment/tangram_H.png") |> 
  filter(gameId=="BTbGhXZvjdSFubTBg") |>   select(playerId, repNum, text, countCorrect, role) |> filter(repNum>3)

listener_sample <- pre_chat_3 |> filter(target=="/experiment/tangram_H.png") |> 
  filter(gameId=="2nZ79PA8WMtYNqLrS") |>   select(playerId, repNum, text, countCorrect, role) |> filter(repNum>3)
...
Overall, we found that larger groups displayed a higher proportion of trials where at least one matcher produced utterances (Supplement Figure 3A, `r stats(anylist_1, 4)`), which declined across repetition blocks (`r stats(anylist_1, 1)`).
The length of matcher interjections also decreased over time for all games (Supplement Figure 3B, `r stats(list_1,1)`), consistent with the need for early matcher involvement in establishing referential conventions. 
