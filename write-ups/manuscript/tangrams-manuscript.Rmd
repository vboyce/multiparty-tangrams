...
# Supplemental all the models & when they were pre-reg'd
```{r}
form <- function(model){
 dep <- as.character(model$formula[2])
 ind <- as.character(model$formula[3])
...
kable(acc_1 |> select(Term, Est.=Estimate, CrI=`Credible Interval`), 
      caption=str_c("Experiment 1 logistic model of listener accuracy:\\\\\ ",form(acc_spec_1)),
      align='lll')

#TODO figure out how to do non-breaking spaces in the caption since nbsp; doesn't work and neither does ~
```
