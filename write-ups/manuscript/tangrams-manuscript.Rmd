...
## Group Performance
Before looking at the language used in groups, we first check group performance. 

We characterize group performance along two complementary metrics: (1) matcher accuracy and (2) describer efficiency. <!--These two metrics are commonly used to assess whether groups are forming conventions in iterated reference games (CITATIONS GO HERE). -->
...
Again, there was no reliable effect of group size in Experiment 1 (SI Table 2), although 6-player thick games were slower to improve than 2-player thick games (`r stats(acc_3, 3)`). 
...
**TODO this is now truthful, but not very punchy**
...
First, as predicted, describers in larger groups used longer descriptions at the outset than describers in smaller groups (Figure \@ref(fig:behavioral)D-F). This held for the continuous measure of group size for Experiment 1 (`r stats(red_1,4)`) and the 2-person versus 6-person groups in Experiment 3 (`r stats(red_3,7)`).
<!--**(TODO: these results would be much cleaner with a unified meta-analytic model combining data across all three experiments)**-->
Second, although communicative efficiency improved at a similar rate across group size in Experiment 1 (`r stats(red_1,2)`), in Experiment 3, larger groups reduced faster than smaller ones in Experiment 3 (`r stats(red_3, 4)`). However, the faster reduction did not fully make up for the initial longer utterances, so across both experiments smaller games tended to use shorter descriptions. 
...
Overall, describers on average decreased their descriptions by a few words each repetition block, ranging from `r stats_text(red_2a,1)` words in 6-player same describer to  `r stats_text(red_2c,1)` words in 6-player thin. 

**TODO punch back up again?**In sum, while larger groups of matchers initially require more information from the describer, all conditions allowed groups to reduce to more efficient referential conventions. <!-- interaction structure modulates the ability of the group to converge on an efficient set of referential conventions. -->
...
Overall, larger groups displayed a higher proportion of trials where at least one matcher produced utterances (Supplement Figure 2A, `r stats(anylist_1, 4)`), which declined across blocks (`r stats(anylist_1, 1)`). 
...
**Broadly, as groups get larger, describers must provide more information and matchers must provide more feedback to achieve similar accuracy. However, thicker interaction structures ease the burdens of larger group sizes. Six-person groups can only approach the performance of two-person groups when matchers are free to ask specific clarifying questions and describers are able to aggregate information across multiple repetition blocks.**
...
**In the previous sections, we explored the functional challenges facing multi-matcher communication and the solutions afforded by thicker interaction structure.**
...
<!--Initial similarities between groups were the same regardless of group size in Experiment 1 (Figure \@ref(fig:sbert)D, `r stats(div_1, 3,3)`), but 6-player thick games had lower initial similarities than 6-player thin games (Figure \@ref(fig:sbert)F, `r stats(div_1, 3,3)`)-->
...
In Experiment 3, thin 6-person games showed much less group-specific divergence despite comparable accuracy and efficiency. **TODO possibly rephrase so it's clear we aren't proving the null**
...
**TODO: might be worth mentioning that we have a pretty small number of groups per cell? and whatever consequences of this are **
<!-- ## Conclusion  -->
