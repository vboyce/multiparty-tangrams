...
one_sbert_concat <- read_csv(here(rotate_data_loc,"pre_sbert.csv")) |>   bind_cols(readRDS(here(rotate_data_loc,'post_sbert.RData'))  %>% as_tibble())
...
do_converge_diverge <- function(concat){
  F_mat <- concat %>% select(starts_with("V")) %>% as.matrix() #Features
M_mat <- concat %>% select(-starts_with("V")) %>% mutate(feature_ind=row_number()) |> 
  mutate(condition=str_c(playerCond,channelCond))
