...
TODO fix captions on Figures 

TODO apply graphical niceties to supplement images

intro could use more cites 

TODO consistency in captitals and hyphens etc
...
ggplot(aes(x=repNum+1, y=words, color=numPlayers))+
  ...
  guides(color = guide_legend(nrow=2, byrow=T, override.aes = list(linetype = 0, alpha=1, fill=NA, size=5) ) )+
...
### Speaker said more in larger games.
...
The main effect of being one block later was `r stats_text(red_1,1)` words per trial in experiment 1; `r stats_text(red_2a,1)` words for 6 consistent speaker; `r stats_text(red_2b,1)` words for 6 full feedback, `r stats_text(red_2c,1)` words for 6 thin; and `r stats(red_3,1)` for the 2 thick condition of experiment 3. 
...
### Larger groups lead to more listener backchannels. TODO reformat to be closer to other sections!

Listeners talk much less than speakers, and listener contributions are concentrated in early trials. The more listeners, the more likely it was that some listener talked, and the more listeners said if they talked (TODO CROSSREF SUPPLEMENT IMAGE). The number of trials where any listener said anything related to the image was higher in larger groups (expt 1, `r stats(anylist_1, 4)`) and declined across blocks (expt 1, `r stats(anylist_1, 1)`). When referential language was produced, larger groups produced more language (expt 1, `r stats(list_1, 4)`), but the difference in group size closed in later blocks (expt 1, `r stats(list_1, 2)`). 

This pattern is consistent with early listener involvement in establishing a common conceptualization by asking questions and offering alternative descriptions. Once a shared reference description is in place, listener descriptions are rarer and more perfunctory. 

Emoji use is not directly comparable to referential language, but similar trends occurred in the thin games. Emoji use was more common in the 6 player games than the two player games and decreased over the course of the game (SUPP FIGURE). 
...
### Interim summary TODO

Smaller groups show higher within-group similarity and between group-difference, whether as a change over time or from initial starting points. The thicker the games the faster and stronger the divergence and convergence. A strong interaction between thinness and game size, where the combination hampers convergence and divergence much more than is seen in  2-thin and 6-thick. 
