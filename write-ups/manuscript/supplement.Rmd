...
\pagebreak
# Feedback
Figure S1 shows the type of feedback shown to participants in different conditions. 
...
Figure S2 shows the fraction of trials where the speaker used these words. Generally, the more concrete word categories become less frequent over time, although this effect seems weaker in larger, thinner games. 
...
```{r words, fig.width=10, fig.height=12, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = " Fraction of trials where the describer used certain types of words. See text for word lists. Faint dots are per tangram, larger dots and error bars are means and boostrapped 95% CIs. "}
...
In Figure S3, the number of games goes up in some cases because only complete blocks (where the describer said something every trial) are analysed. If there was initial confusion and a describer missed a trial, that block was excluded. 
...
Versions of main paper Figures 2 and 4 with just these complete games are shown here in Figures S4 and S5. The full model output for the sensitivity analyses is in Tables S54-S73. 
...
As the number of words produced by the speaker each trial decreases over repetition, so does the time each trial takes. In Figure S9, the time to selection (per-matcher) is shown in panels A-C, and the time per trial (equivalent to how long the slowest matcher took) is shown in panels B-D. 
...
We also confirm in Figure S10 that there is a strong relationship between the number of words from the describer and the length of a trial. 

```{r timing, fig.env = "figure*", fig.pos = "b!", fig.width=10, fig.height=8.5, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "TODO Behavioral results across all three experiments. (A-C). Matcher accuracy at selecting the target image. (D-F). Number of words produced by the describer each trial. For all, small dots are per game, per block means, and smooth lines are predictions from model fixed effects with 95\\% credible intervals. Y-axes are truncated, and a few outliers points are not visible."}
