...
combined_emoji |> mutate(emoji=case_when(
  ...
  mutate(cond=case_when(
    condition=="2 thin" ~ "Expt 3: 2 thin",
    condition=="6 thin" ~ "Expt 3: 6 thin",
...
```{r other, fig.env = "figure*", fig.pos = "t!", fig.width=10, fig.height=8, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "Additional measures of convergence and divergence. A is similarity to first utterance. B is similarity between utterances from adjacent blocks. ", cache=T}
...
```{r dist, fig.env = "figure*", fig.pos = "t!", fig.width=10, fig.height=4, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "Divergence in descriptions of different tangrams within a group", cache=T}
...
For experiment 1, we also pre-specified a model about whether the speaker's correctness (as a listener) on the prior block had an effect on how many words of description they produced. 
...
We also looked at how similar an utterance was to the first round utterance. This is not very informative because first round utterances tend to be pretty unwieldy. 
