...
As an exploratory, qualitative analysis of the describer's language, we performed dictionary searches to classify whether the describers utterances contained the following types of words. 
...
Qualitatively, descriptions that contain none of these words are usually shorter, abstract, more conventionalized descriptions. 
...
```{r words, fig.width=10, fig.height=12, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "TODO Fraction of trials where the describer used the following types of words. "}
...
The number of games in each condition varied due to the realities of online recruitment for real-time games. We had control over how many participants we recruited on Prolific. Because of the real-time nature of the games, we relied on the fact that most Prolific workers who accepted the task would rapidly open it and go to our experiment site, where they would read the instructions and be assigned into games. Due to technical details about how games filled, we only ran one condition of games at any one time. In experiments 1 and 2, games would only start if the game was full. Thus, if for example, we recruited 28 people, and 27 people went to the experiment when we were running a 4-person game, three people would be unable to make a complete game, and we also wouldn't have groupmates available for a 28th person who opened the experiment late. 
...
As a (post-hoc) sensitivity analysis, we re-ran our main analyses on the subset of games that completed all 6 blocks with a full set of players. This is a check on whether any of the results are driven by a) partial games that were included in early blocks but not later blocks or b) experiment 3 games that started without a full set of players or had players drop out. 
...
The only qualitative differences in the results between these models and those on the full dataset are in experiment 3.  Note that large experiment 3 games were the ones most likely to be excluded, both for incompleteness or for not having the full 6 players throughout. 
...
As the number of words produced by the speaker each trial decreases over repetition, so does the time each trial takes. In TODO Figure, the time to selection (per-matcher) is shown in panels A-C, and the time per trial (equivalent to how long the slowest matcher took) is shown in panels B-D. 

Matchers speed up over time across conditions, although thin large groups may not speed up as much. Larger groups appear to be a bit slower at the level of individual matchers, which compounds to overall slower games as there are more matchers to wait for. 
...
```{r timing-words, fig.env = "figure*", fig.pos = "t!", fig.width=10, fig.height=4.5, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "Relationship between number of words produced by describer and trial length. Across conditions, a similar pattern holds. X-axes are truncated, and a few outliers points are not visible."}
...
```{r grouping, fig.env = "figure*", fig.pos = "t!", fig.width=9, fig.height=10, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "Comparison between the population-level coefficients (point estimate and 95% CI) and the group-level standard deviations (point estimates and 95% CIs) for the heirarchical effects. "}
mixed_path <- "code/paper_mods/mixed_fx"
...
do_plot <- function(modela, modelb, titlea, titleb) {
  a <- modela |>
    ...
    scale_color_manual(values = colors)+
    labs(x="Coefficient estimates", title=titlea)
  b <- modelb |>
    mutate(Term = relevel(factor(Term), "Intercept")) |>
    ggplot(aes(x = Term, color = group, y = Estimate, ymin = lower, ymax = upper)) +
    coord_flip() +
    ...
    theme(legend.position = "bottom", 
          ...
          axis.text=element_text(size=10)) +
    scale_color_manual(values = colors)+
        labs(x="Coefficient estimates", title=titleb)
