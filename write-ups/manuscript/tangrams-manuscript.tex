...
\item
  Clean up supp figs
\item
  Consider clean up more of real figs
\item
  Reoutline takeaways, propogate to set up/intro, results, GD
\item
  note in supp that we 0 indexed block in models
\item
...
Listener accuracy rose over repetitions in all games, and approached ceiling in most conditions; however, 6 player games with limited communication and feedback were less accurate (Figure \ref{fig:behavioral}A).

In every condition, listeners selected the correct target far above chance, and listener accuracy rose over the course of the game . Experiment 1 found no strong effects of group size on overall accuracy (numPlayers: -0.07 {[}-0.2, 0.05{]}) or improvement rate (block:numPlayers: -0.02 {[}-0.05, 0.01{]}).
...
There's likely a tradeoff between accuracy and reduction: for a given set of listeners, it may be possible to explain more extensively and increase accuracy (but with more words) or the explain less but risk that not everyone understands. Groups seem to have gone the explaining more path, especially when backchannel was available.
...
Listeners do not talk as much as speakers, but they also show patterns of reduction. In experiment 1, the number of trials where any listener says anything related to the description of the image is higher in larger groups (numPlayers: 0.78 {[}0.58, 0.98{]}), and declines across blocks (block: -0.8 {[}-0.97, -0.63{]}). When referential language is produced, more is produced in larger groups (numPlayers: 2.12 {[}1.03, 3.12{]}), but the difference in group size closes in later blocks (block:numPlayers: -0.41 {[}-0.72, -0.09{]}). See TODO supplement for images.

TODO fix this sentence b/c too long
This pattern of listener language use is suggestive that listeners asking questions and offering alternative conceptualizations plays a role in initially establishing a shared description in early blocks, but they don't need to contribute referential language in later blocks when the description is reducing.
...
While the volume of listener contributions is low, the substantial differences in accuracy between thick and thin games suggest that these listener contributions have a crucial role to play. NOte that the differences in accuracy don't actually show up as significant in model!
Listeners also produced less language over the duration of games, consistent with the idea that backchannels were used primarily earlier in games before conventions emerged.

\hypertarget{linguistic-content-analyses}{%
\subsection{Linguistic content analyses}\label{linguistic-content-analyses}}

Reduction isn't just about speaker's saying less, its about the formation of a partner-specific conceptualization of the target that leads to an efficient way to refer to it. As a group converges to a nickname for a target, we would predict that the descriptions would become more similar to the eventual convention (and more similar to each other). The partner specificity means that it's not that everyone figures out to drop hedges and use the same description, but that different groups latch onto different features as the key concept. Thus, we expect different groups to diverge in their descriptions. Also expect that within a group, the descriptions for two different images will get more different over time.
...
These two metrics are illustrated with examples in Figure \ref{fig:sbert-diagram}.
