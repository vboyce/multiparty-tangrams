...
## Data analysis
The first author skimmed through the chat transcripts, tagging utterances that did not refer to the current tangram. These were primarily pleasantries ("Hello"), meta-commentary about well or fast the task was going and confirmations or denials ("ok", "got it", "yes", "no"). We exclude these utterances from our analyses.
...
```{r accuracy, out.height="25%", fig.width=6, fig.height=4, fig.pos='h', fig.cap="(ref:accuracy)"}
...
```
Look accuracy \@ref(fig:accuracy) Most groups were accurate in their selections, with accuracy rising over rounds

(ref:time) TODO

```{r time, out.height="25%", fig.width=6, fig.height=4, fig.pos='h', fig.cap="(ref:time)"}
...
Look time \@ref(fig:time)


(ref:total-words) The total number of words speakers and listeners said each block. Length of utterances decreases over time. Note: y-axis clipped at 50 which hides a few speaker outliers. 

```{r total-words, out.height="25%", fig.width=6, fig.height=4, fig.pos='h', fig.cap="(ref:total-words)"}
...
```
Look words \@ref(fig:total-words)



(ref:word-lines) TODO

```{r variability, out.height="25%", fig.width=6, fig.height=4, fig.pos='h', fig.cap="(ref:word-lines)"}
...
```
Look words \@ref(fig:variability)
