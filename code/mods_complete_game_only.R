...
model_location <- "code/paper_mods/complete_only"
# Read data
...
include_1 <- include |> ungroup() |> select(gameId_1 = gameId)
include_2 <- include |> ungroup() |> select(gameId_2 = gameId)
...
one_two_converge <- read_rds(here("code/models/one_two_converge.rds")) |> 
  inner_join(include |> ungroup() |>  select(gameId))
...
model_1_to_last <- brm(sim ~ earlier * condition + (1 | tangram) + (1 | gameId),
  data = one_two_converge |> filter(condition %in% c("2", "3", "4", "5", "6")) |> 
...
# skipping "additional" analyses (not in main paper) for this sensitivity analysis
