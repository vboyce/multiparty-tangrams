...
blah <- d.chat %>% filter(role=="speaker") %>% 
  group_by(repNum, numPlayers, gameId) %>% 
  summarize(words=sum(total_num_words))
ggplot(blah, aes(x=repNum, y=words, color=as.factor(numPlayers), group=gameId))+
  facet_wrap(~numPlayers, nrow=1)+
  #facet_wrap(~tangram)+
  scale_color_brewer(palette="Dark2")+
    geom_line(alpha=.3,method=glm, se=F)+
     #stat_summary(fun.data = "mean_cl_boot")+
  labs(title="Number of words", y="Number of words", x="Round number")+
  theme(legend.position="bottom")
