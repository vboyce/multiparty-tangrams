...
```{r}
for_model <- d.chat %>% filter(role=="speaker") %>% 
  select(total_num_words, repNum, tangram, numPlayers)
crappy_model <- lmer(total_num_words~repNum*numPlayers+(1|tangram), for_model)

summary(crappy_model)
```
...
ggsave(here(image_location, 'words_lines.pdf'), width=6, height=3)
...
ggsave(here(image_location, 'words_tangrams.pdf'), width=8, height=5)
...
ggplot(d.chat.lagged, aes(x=repNum, y=total_num_words, color=as.factor(numPlayers)))+
  facet_wrap(~prev_correct_round, nrow=1) +
    scale_color_brewer(palette="Dark2")+
      #geom_smooth(method=lm)+
      stat_summary(fun.data = "mean_cl_boot")+
    labs(title="Utterance length", y="Number of words", x="Round number")

ggplot(d.chat.lagged, aes(x=repNum, y=reduction_word, color=as.factor(numPlayers)))+
  facet_wrap(~prev_correct_round, nrow=1) +
    scale_color_brewer(palette="Dark2")+
      #geom_smooth(method=lm)+
      stat_summary(fun.data = "mean_cl_boot")+
