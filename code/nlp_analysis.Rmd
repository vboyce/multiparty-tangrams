...
speaker_collapsed <- exchanges %>% 
  filter(role=="speaker") %>% 
  group_by(gameId, targetNum, repNum,trialNum,numPlayers,playerId, target, countCorrect) %>%
  summarize(utterance = paste0(utterance, collapse = ' ')) %>% 
...
NB: haven't done any spell check or good stop word fixing
...
library(reticulate)
np <- import("numpy")
mat = np$load(here('data/study1/content/feats_tangrams_embeddings_rawavg.npy'))
saveRDS(mat, here('data/study1/content/feats_tangrams_embeddings_rawavg.RData'))
```

```{r}
M_mat = read_csv(here('data/study1/content/meta_tangrams_embeddings.csv'), 
                na = c('[nan]'), quote = '"') %>%  
  mutate(feature_ind = row_number()) %>%
  select(-X1) 

F_mat <- readRDS(here('data/study1/content/feats_tangrams_embeddings_rawavg.RData'))
...
Note: speaker is guaranteed to change for n-n+1, so predictors are numPlayers, round and 1|group, 1|target
