...
matches %>% filter(later_rep==5) %>% filter(word=="karate") %>% filter(earlier_rep==0) %>% filter(gameId=="2928FF7GcrkeFiDkh") %>% filter(target=="/experiment/tangram_A.png")
...
san_check <- foobar %>% group_by(gameId,numPlayers) %>% summarize(san=sum(pct))
ggplot(foobar, aes(x=earliest_rep, y=pct, color=as.factor(numPlayers)))+geom_jitter(alpha=.5,width=.2, height=0)+facet_grid(.~numPlayers)+
    stat_summary(fun.data = "mean_cl_boot", position = position_dodge(width=.2), color="black")+
  theme(legend.position="bottom")
```
