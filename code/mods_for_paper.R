...
lagged <- combined_results |>
  filter(condition == "rotate") |>
  select(playerId, gameId, tangram, repNum, correct) |>
  mutate(repNum = repNum + 1)

for_weird_model <- combined_chat |>
  filter(role == "speaker") |>
  filter(condition == "rotate") |>
...
model_speaker_acc <- brm(
  words ~ block * numPlayers + block * was_INcorrect +
...
draws <- m |>
  spread_draws(
    `b_block:channelthin`, b_block,
    `b_block:gameSize6`, `b_block:channelthin:gameSize6`
  ) |>
  mutate(
    thin6 = `b_block:channelthin` + `b_block:gameSize6` + b_block + `b_block:channelthin:gameSize6`,
