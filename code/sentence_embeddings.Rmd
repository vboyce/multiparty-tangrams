...
colors=c("#FDE725FF","#5DC863FF", "#21908CFF", "#3B528BFF", "#440154FF", "#eb34eb","#e04b14", "#ff0000")
...
pre_sbert <- raw_text %>%union(noro) %>% union(high_feed) %>% union(emoji) %>% write_csv("pre_sbert.csv")
```
...
#sample <- raw_text %>% filter(gameId=="3WzEi9zkHF77vdTw5") %>% write_csv("sample.csv")
pre_sbert_concat <- raw_text %>%union(noro) %>% union(high_feed) %>% union(emoji) %>%  write_csv("pre_sbert_concat.csv")
```
...
```{r, fig.width=4, fig.height=2.5, include=F, eval=F}
ggplot(game_divergence, aes(x=repNum+1,y=sim,color=as.factor(numPlayers)))+
      stat_summary(aes(group=str_c(numPlayers,tangram), color=as.factor(numPlayers)),fun.data = "mean_cl_boot", position = position_dodge(width=.3), alpha=.5, geom="point")+

  geom_smooth(formula=y~poly(x,2))+

  scale_x_continuous(breaks=seq(1,6))+
  labs(y="Cosine Similarity", x="Block", color="Players")+
  theme(text=element_text(size=14), legend.position="bottom")+
    scale_color_viridis(discrete=T, direction=-1)
  #coord_cartesian(ylim=c(0.1,.6))

ggsave(here(image_location, "sbert_diverge.png"))
```
...
```{r, fig.width=5, fig.height=2.5, include=F, eval=F}
ggplot(tangram_distinctive, aes(x=repNum+1,y=sim,color=as.factor(numPlayers)))+
      stat_summary(aes(group=str_c(numPlayers,gameId), color=as.factor(numPlayers)),fun.data = "mean_cl_boot", position = position_dodge(width=.3), alpha=.5, geom="point")+
...
ggsave(here(image_location, "sbert_diverge_within.png"))
```
<!--## Adjacent -->
...
```{r fig.width=5, fig.height=2.5, eval=F, include=F}

to_first <- tangram_change %>% filter(earlier==0) %>% ungroup()
left <- ggplot(to_first, aes(x=later+1,y=sim,color=as.factor(numPlayers)))+
      stat_summary(aes(group=str_c(numPlayers,gameId), color=as.factor(numPlayers)),fun.data = "mean_cl_boot", position = position_dodge(width=.3), alpha=.5, geom="point")+
  geom_smooth(formula=y~poly(x,2))+
  scale_x_continuous(breaks=seq(1,6))+
  labs(y="Cosine Similarity to ...", x="Block",title="first block")+
  theme(text=element_text(size=14),legend.pos="none", plot.title=element_text(size=14, hjust=.5, vjust=.5, margin=margin(t=40, b=-30)))+
    scale_color_viridis(discrete=T, direction=-1)+
  coord_cartesian(ylim=c(0.2,1))

#ggsave(here(image_location, "sbert_tofirst.png"))

middle <- ggplot(adjacent, aes(x=earlier+1,y=sim,color=as.factor(numPlayers)))+
      stat_summary(aes(group=str_c(numPlayers,gameId), color=as.factor(numPlayers)),fun.data = "mean_cl_boot", position = position_dodge(width=.3), alpha=.5, geom="point")+
  geom_smooth(formula=y~poly(x,2))+
  scale_x_continuous(breaks=seq(1,6))+
  labs(y="", x="Block")+
  ggtitle("next block")+
  theme(text=element_text(size=14), legend.pos="none", plot.title=element_text(size=14,hjust=.5, vjust=.5, margin=margin(t=40, b=-30)), axis.title.y=element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank())+
    scale_color_viridis(discrete=T, direction=-1)+
  coord_cartesian(ylim=c(0.2,1))
...
```{r fig.width=4, fig.height=2.5, eval=F, include=F}
 ggplot(to_last, aes(x=earlier+1,y=sim,color=as.factor(numPlayers)))+
      stat_summary(aes(group=str_c(numPlayers,gameId), color=as.factor(numPlayers)),fun.data = "mean_cl_boot", position = position_dodge(width=.3), alpha=.5, geom="point")+
  geom_smooth(formula=y~poly(x,2))+
  scale_x_continuous(breaks=seq(1,6))+
  labs(y="Cosine Similarity", x="Block", color="Players", title="Similarity to description in final block")+
  theme(text=element_text(size=14), plot.title=element_text(size=14,hjust=.5, vjust=.5, margin=margin(t=40, b=-30)))+
    scale_color_viridis(discrete=T, direction=-1)+
  coord_cartesian(ylim=c(0.2,1))

ggsave(here(image_location, "sbert_tolast.png"))

```

```{r eval=F, include=F}
 square_converge <- to_last |> filter(numPlayers %in% c("2","3","4","5","6")) |> ggplot( aes(x=earlier+1,y=sim,color=as.factor(numPlayers)))+
      stat_summary(aes(group=str_c(numPlayers,gameId), color=as.factor(numPlayers)),fun.data = "mean_cl_boot", position = position_dodge(width=.3), alpha=.5, geom="point")+
  geom_smooth(formula=y~poly(x,2))+
  scale_x_continuous(breaks=seq(1,6))+
  labs(y="Cosine Similarity", x="Block", color="Players")+
...
library(cowplot)
save_plot(here(image_location,"square_converge.pdf"),square_converge, base_width=5, base_height=5)
...
yaxis <- get_y_axis(left)
plots <- plot_grid(left,
                   NULL,
                   middle,
                   NULL,
                   right+theme(legend.position = "none"),
                   nrow=1, rel_widths=c(7.5,-.2,6,-.2,6))

plot_grid(plots, legend, nrow=1, rel_widths=c(1,.1))
...
ggplot(game_divergence, aes(x=repNum,y=sim,color=as.factor(numPlayers)))+
        stat_summary(aes(group=str_c(numPlayers,tangram), color=as.factor(numPlayers)),fun.data = "mean_cl_boot", position = position_dodge(width=.3), alpha=.5, geom="point")+geom_smooth(formula=y~poly(x,2))+
...
ggplot(tangram_distinctive, aes(x=repNum,y=sim,color=as.factor(numPlayers)))+
      stat_summary(aes(group=str_c(numPlayers,gameId), color=as.factor(numPlayers)),fun.data = "mean_cl_boot", position = position_dodge(width=.3), alpha=.5, geom="point")+geom_smooth(formula=y~poly(x,2))+
    scale_color_manual(values=colors)
```
...
scale_color_manual(values=colors)+  facet_grid(.~has.speaker)
