...
test %>% nrow() #15040 total
test %>% filter(n>1) %>% nrow() # 7565
...
About half of person-trial combos are single lines already, so the will have identical embeddings between the two methods. Of the other half, this is the cosine similarity distribution between the two methods (grouped by player count). 
...
Similarity is between that rep and the next later.
